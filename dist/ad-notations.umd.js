(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('break_eternity.js'), require('tslib')) :
  typeof define === 'function' && define.amd ? define(['exports', 'break_eternity.js', 'tslib'], factory) :
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.ADNotations = {}, global.Decimal, global.tslib));
  ...
  var Settings = {
    isInfinite: function isInfinite(decimal) {
      return decimal.gte(new Decimal__default["default"]("1e9000000000000000"));
    },
    exponentCommas: {
      show: true,
      min: 100000,
  ...
  var StandardNotation = function (_super) {
    ...
    StandardNotation.prototype.formatDecimal = function (value, places, placesExponent) {
      return formatMantissaWithExponent(formatMantissaBaseTen, abbreviateStandard, 1000, 1, undefined, " ", true)(value, places, placesExponent);
    };
  ...
  var LogarithmNotation = function (_super) {
    ...
    LogarithmNotation.prototype.formatDecimal = function (value, places, placesExponent) {
      var log10 = value.max(1).log10().toNumber();
  ...
  var BracketsNotation = function (_super) {
    ...
    BracketsNotation.prototype.formatDecimal = function (value) {
      var table = [")", "[", "{", "]", "(", "}"];
      var log6 = Math.LN10 / Math.log(6) * value.max(1).log10().toNumber();
  ...
  var InfinityNotation = function (_super) {
    ...
    InfinityNotation.prototype.formatDecimal = function (value, places) {
      var infinities = value.log(Number.MAX_VALUE);
  ...
  var DotsNotation = function (_super) {
    ...
    DotsNotation.prototype.formatDecimal = function (value) {
      ...
      var log = value.max(1).log10().toNumber() / Math.log10(254);
  ...
  var ZalgoNotation = function (_super) {
    ...
    ZalgoNotation.prototype.heComes = function (value) {
      var scaled = (value.plus(1).exponent + Math.log(Math.max(value.mantissa, 1))) / 66666 * 1000;
  ...
  var HexNotation = function (_super) {
    ...
    HexNotation.prototype.modifiedLogarithm = function (x) {
      var floorOfLog = Math.floor(Decimal__default["default"].max(x, 1).log10().toNumber() / Math.log10(2));
  ...
  var ImperialNotation = function (_super) {
    ...
    ImperialNotation.prototype.formatDecimal = function (value) {
      ...
      var logValue = new Decimal__default["default"](10).pow(value.max(1).log10().min(LOG_MAX_VOLUME)).exponent + Number(Math.log(Math.max(new Decimal__default["default"](10).pow(value.max(1).log10().min(LOG_MAX_VOLUME)).mantissa, 1)));
  ...
  var ClockNotation = function (_super) {
    ...
    ClockNotation.prototype.clockwise = function (value) {
      ...
      var log = value.max(1).log10().toNumber() / LOG12;
  ...
  var PrimeNotation = function (_super) {
    ...
    PrimeNotation.prototype.primify = function (value) {
      ...
      var exp = (value.exponent + Math.log(Math.max(value.mantissa, 1))) / MAX_INT_LOG_10;
  ...
  var BarNotation = function (_super) {
    ...
    BarNotation.prototype.formatDecimal = function (value) {
      ...
      var log8 = Math.LN10 / LOG8 * value.max(1).log10().floor().toNumber();
  ...
  var ShiNotation = function (_super) {
    ...
    Object.defineProperty(ShiNotation.prototype, "infinite", {
      get: function get() {
        return this.shi(new Decimal__default["default"](Number.MAX_VALUE));
      },
      enumerable: false,
      configurable: true
    });
    ...
    ShiNotation.prototype.shi = function (value) {
      var scaled = Math.pow(value.plus(1).max(1).log10().floor().toNumber() * 1000, 0.08);
      var shi = "";
  ...
  var AllNotation = function (_super) {
    ...
    AllNotation.prototype.formatDecimal = function (value, places) {
      var index = Math.floor(Math.log2(value.abs().plus(2).max(1).log10().toNumber() / Math.log10(2)));
