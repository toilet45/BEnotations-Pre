...
var HexNotation = function (_super) {
  ...
  HexNotation.prototype.modifiedLogarithm = function (x) {
    var floorOfLog = Math.floor(Decimal.max(x, 1).log10().toNumber() / Math.log10(2));
...
var ImperialNotation = function (_super) {
  ...
  ImperialNotation.prototype.formatDecimal = function (value) {
    ...
    var logValue = new Decimal(10).pow(value.max(1).log10().min(LOG_MAX_VOLUME)).exponent + Number(Math.log(Math.max(new Decimal(10).pow(value.max(1).log10().min(LOG_MAX_VOLUME)).mantissa, 1)));
...
var ShiNotation = function (_super) {
  ...
  Object.defineProperty(ShiNotation.prototype, "infinite", {
    get: function get() {
      return this.shi(new Decimal(Number.MAX_VALUE));
    },
    enumerable: false,
    configurable: true
  });
